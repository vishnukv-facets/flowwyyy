...
// stubSlackDoneNoticeRunner swaps the slackDoneNoticeRunner with a recorder
// that returns retErr for every call. Mirrors stubClaudeRunner. Returns a
// pointer to the captured-call slice so the test can assert.
func stubSlackDoneNoticeRunner(t *testing.T, retErr error) *[]capturedSlackNotice {
	t.Helper()
	old := slackDoneNoticeRunner
	calls := &[]capturedSlackNotice{}
	slackDoneNoticeRunner = func(_ context.Context, channel, threadTS, text string) error {
		*calls = append(*calls, capturedSlackNotice{channel: channel, threadTS: threadTS, text: text})
		return retErr
	}
	t.Cleanup(func() { slackDoneNoticeRunner = old })
	return calls
}

// seedSlackOriginAction wires a Slack monitor event + action row to a task
// so postSlackDoneNotice's join against monitor_event_actions resolves.
func seedSlackOriginAction(t *testing.T, db *sql.DB, taskSlug, channel, ts, threadTS string) {
	t.Helper()
	id := flowdb.MonitorEventID("slack", channel+":"+ts)
	now := flowdb.NowISO()
	rawJSON := `{"conversation":{"id":"` + channel + `"},"message":{"ts":"` + ts + `"`
	if threadTS != "" {
	...
	if err := flowdb.RecordMonitorEventAction(db, id, "spawn", taskSlug, ""); err != nil {
		t.Fatalf("seed action: %v", err)
	}
}

func TestPostSlackDoneNoticeOnSlackOriginTask(t *testing.T) {
	setupFlowRoot(t)
	calls := stubSlackDoneNoticeRunner(t, nil)
	// FLOW_BASE_URL pinned so the test asserts the exact URL the user
	// will see; absent env we'd test the default 8787 port which obscures
	// the intent.
	t.Setenv("FLOW_BASE_URL", "http://flow.example/")

	if rc := cmdAdd([]string{"task", "Patch latency bug", "--slug", "slack-task"}); rc != 0 {
		t.Fatalf("add rc=%d", rc)
	}
	db := openFlowDB(t)
	defer db.Close()
	// Pre-seed a session_id so cmdDone's gate doesn't reject the flip.
	if _, err := db.Exec(
		`UPDATE tasks SET session_id='aaaaaaaa-aaaa-4aaa-aaaa-aaaaaaaaaaaa', session_started=?, status='in-progress' WHERE slug='slack-task'`,
		flowdb.NowISO(),
	); err != nil {
		t.Fatal(err)
	}
	seedSlackOriginAction(t, db, "slack-task", "D9", "1710000001.000001", "")

	task, err := flowdb.GetTask(db, "slack-task")
	if err != nil {
		t.Fatal(err)
	}
	if err := postSlackDoneNotice(db, task); err != nil {
		t.Fatalf("postSlackDoneNotice err = %v", err)
	}
	if len(*calls) != 1 {
	...
	if got.threadTS != "1710000001.000001" {
		t.Errorf("threadTS = %q, want top-level ts promoted to thread parent", got.threadTS)
	}
	wantSubstr := []string{
		`"Patch latency bug"`,
		"http://flow.example/tasks/slack-task",
	}
	...
	// FLOW_BASE_URL had a trailing slash — make sure flowBaseURL stripped it
	// so we don't generate //tasks/slug.
	if contains(got.text, "flow.example//tasks") {
		t.Errorf("trailing slash not trimmed: %s", got.text)
	}
}

func TestPostSlackDoneNoticeNoOpForNonSlackOriginTask(t *testing.T) {
	setupFlowRoot(t)
	calls := stubSlackDoneNoticeRunner(t, nil)
	if rc := cmdAdd([]string{"task", "Adhoc work", "--slug", "adhoc-task"}); rc != 0 {
		t.Fatalf("add rc=%d", rc)
	}
	db := openFlowDB(t)
	defer db.Close()
	task, _ := flowdb.GetTask(db, "adhoc-task")
	if err := postSlackDoneNotice(db, task); err != nil {
		t.Fatalf("err = %v", err)
	}
	if len(*calls) != 0 {
		t.Errorf("calls = %d, want 0 (no slack origin)", len(*calls))
	}
}

func TestPostSlackDoneNoticeWarnsButDoesntFailCmdDone(t *testing.T) {
	setupFlowRoot(t)
	stubClaudeRunner(t, nil)
	// Stub returns an error — cmdDone must still return 0. The Slack post
	// is best-effort; failure must not roll back the status flip.
	stubSlackDoneNoticeRunner(t, errors.New("simulated slack outage"))

	if rc := cmdAdd([]string{"task", "Outage drill", "--slug", "outage-drill"}); rc != 0 {
		t.Fatalf("add rc=%d", rc)
	}
	db := openFlowDB(t)
	if _, err := db.Exec(
		`UPDATE tasks SET session_id='bbbbbbbb-bbbb-4bbb-bbbb-bbbbbbbbbbbb', session_started=?, status='in-progress' WHERE slug='outage-drill'`,
		flowdb.NowISO(),
	); err != nil {
		t.Fatal(err)
	}
	seedSlackOriginAction(t, db, "outage-drill", "D1", "1.0001", "")
	db.Close()

	if rc := cmdDone([]string{"outage-drill", "--no-pr"}); rc != 0 {
		t.Fatalf("cmdDone rc = %d, want 0 (slack failure must not roll back)", rc)
	}
	db = openFlowDB(t)
	defer db.Close()
	task, _ := flowdb.GetTask(db, "outage-drill")
	if task.Status != "done" {
		t.Errorf("status = %q, want done despite slack failure", task.Status)
	}
}
