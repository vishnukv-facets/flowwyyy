...
func (b *SlackBackfill) reconcile(ctx context.Context, slug, channel, threadTS string) (int, error) {
	maxTS, seen, err := inboxSlackTSIndex(slug)
	if err != nil {
		return 0, err
	}
...
// inboxSlackTSIndex reads a task's inbox.jsonl once and returns the newest
// Slack message ts present (the resume cursor) and the set of all message ts,
// for dedup.
func inboxSlackTSIndex(slug string) (maxTS string, seen map[string]bool, err error) {
