...
func GetThreadState(db *sql.DB, threadKey string) (ThreadState, bool, error) {
	...
	if err != nil {
		return ThreadState{}, false, fmt.Errorf("flowdb: get thread state %q: %w", threadKey, err)
	}
	...
	if err := json.Unmarshal([]byte(actionsJSON), &s.OperatorActions); err != nil {
		return ThreadState{}, false, fmt.Errorf("flowdb: thread state %q operator_actions: %w", threadKey, err)
	}
...
func RecordThreadDecision(db *sql.DB, d ThreadDecision) error {
	if d.ThreadKey == "" {
		return fmt.Errorf("flowdb: thread decision requires a thread_key")
	}
	_, err := db.Exec(
	...
	if err != nil {
		return fmt.Errorf("flowdb: record thread decision %q: %w", d.ThreadKey, err)
	}
	return nil
}
...
func appendThreadJSON(db *sql.DB, threadKey, column string, entry any, at string) error {
	...
	if _, err := db.Exec(
		...
		return fmt.Errorf("flowdb: ensure thread state %q: %w", threadKey, err)
	}
	var raw string
	if err := db.QueryRow(`SELECT `+column+` FROM attention_thread_state WHERE thread_key = ?`, threadKey).Scan(&raw); err != nil {
		return fmt.Errorf("flowdb: load thread %s for %q: %w", column, threadKey, err)
	}
	var arr []json.RawMessage
	if err := json.Unmarshal([]byte(raw), &arr); err != nil {
		return fmt.Errorf("flowdb: decode thread %s for %q: %w", column, threadKey, err)
	}
	enc, err := json.Marshal(entry)
	if err != nil {
		return fmt.Errorf("flowdb: encode thread %s entry: %w", column, err)
	}
	arr = append(arr, enc)
	out, err := json.Marshal(arr)
	if err != nil {
		return fmt.Errorf("flowdb: encode thread %s for %q: %w", column, threadKey, err)
	}
	if _, err := db.Exec(
		`UPDATE attention_thread_state SET `+column+` = ?, updated_at = ? WHERE thread_key = ?`,
		string(out), at, threadKey,
	); err != nil {
		return fmt.Errorf("flowdb: append thread %s for %q: %w", column, threadKey, err)
	}
	return nil
}
