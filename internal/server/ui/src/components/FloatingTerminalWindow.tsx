...
import type { FloatingSession } from '../lib/types'
...
export function FloatingTerminalWindow({ session, pos, z, hidden, onMove, onFocus, onMinimize, onClose }: Props) {
  ...
  const style = { left: at.x, top: at.y, zIndex: z } as CSSProperties
    ...
    >
      ...
      >
        <div className="floating-terminal-title">
          ...
          <span className="clip">{session.title || 'Ask Flow'}</span>
        </div>
        ...
        <button type="button" className="btn icon sm" title="End session" onClick={onClose}>
          <X size={15} />
        </button>
      </div>
