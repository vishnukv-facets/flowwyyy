...
var settingsRegistry = []settingSpec{
	...
	{Key: "FLOW_BACKUP_OFFSITE", Label: "Offsite backup", Group: "General", Type: settingString, Default: "auto", Help: "auto: when a personal GitHub token is available, flow backs up to a PRIVATE flow-backup repo in your PERSONAL GitHub account (the account the token belongs to) — never an org, even if the GitHub App connector is installed on one (the App can't create a personal repo). Set the token on the Knowledge → Backups panel, or via GITHUB_TOKEN/GH_TOKEN/FLOW_BACKUP_TOKEN / the gh CLI. local: keep backups on this machine only. Either way, local versioning + db snapshots always run."},
}
