...
func TestBackfillFeedTaskThreadTags(t *testing.T) {
	...
	// Bucket-O tag writes route through `flow update task --tag` (taskTagger); no
	// flow binary in this unit test, so stub it to perform the same DB effect.
	orig := taskTagger
	defer func() { taskTagger = orig }()
	taskTagger = func(_ context.Context, slug, tag string) error { return flowdb.AddTaskTag(db, slug, tag) }
...
func TestReconcileOpenFeedMatches(t *testing.T) {
	...
	got, _ := productdb.GetFeedItem(db, "rc1")
	...
	still, _ := productdb.GetFeedItem(db, "rc2")
...
func TestDismissSurfacedDropCards(t *testing.T) {
	...
	if got, _ := productdb.GetFeedItem(db, "d1"); got.Status != "dismissed" {
		t.Errorf("d1 status = %q, want dismissed", got.Status)
	}
	if got, _ := productdb.GetFeedItem(db, "r1"); got.Status != "new" {
		t.Errorf("r1 (reply) status = %q, want new (untouched)", got.Status)
	}
	if newItems, _ := productdb.ListFeedItems(db, "new"); len(newItems) != 1 {
...
func TestBackfillSkipsDeletedTask(t *testing.T) {
	...
	if tags, _ := productdb.GetTaskTags(db, "att-gone"); len(tags) != 0 {
