...
func InsertChat(db *sql.DB, c Chat) error {
	...
	if err != nil {
		return fmt.Errorf("productdb: insert chat %q: %w", c.Slug, err)
	}
	return nil
}
...
func UpsertChat(db *sql.DB, c Chat) error {
	...
	if err != nil {
		return fmt.Errorf("productdb: upsert chat %q: %w", c.Slug, err)
	}
	return nil
}
...
func SetChatSession(db *sql.DB, slug, sessionID, now string) error {
	...
	if err != nil {
		return fmt.Errorf("productdb: set chat session %q: %w", slug, err)
	}
	return nil
}
...
func SetChatProvider(db *sql.DB, slug, provider, now string) error {
	...
	if err != nil {
		return fmt.Errorf("productdb: set chat provider %q: %w", slug, err)
	}
	return nil
}
...
func SetChatTitle(db *sql.DB, slug, title, now string) error {
	...
	if err != nil {
		return fmt.Errorf("productdb: set chat title %q: %w", slug, err)
	}
	return nil
}
...
func SetChatMuted(db *sql.DB, slug string, mutedAt string) error {
	...
	if err != nil {
		return fmt.Errorf("productdb: set chat muted %q: %w", slug, err)
	}
	return nil
}
...
func ListChats(db *sql.DB, filter ChatFilter) ([]*Chat, error) {
	...
	rows, err := db.Query(q)
	if err != nil {
		return nil, fmt.Errorf("productdb: list chats: %w", err)
	}
	defer rows.Close()
	var out []*Chat
	for rows.Next() {
		c, err := scanChat(rows)
		if err != nil {
			return nil, fmt.Errorf("productdb: scan chat: %w", err)
		}
		out = append(out, c)
	}
	return out, rows.Err()
...
func TouchChat(db *sql.DB, slug, lastActivityAt string) error {
	...
	if err != nil {
		return fmt.Errorf("productdb: touch chat %q: %w", slug, err)
	}
	return nil
}
...
func ArchiveChat(db *sql.DB, slug, at string) error {
	...
	if err != nil {
		return fmt.Errorf("productdb: archive chat %q: %w", slug, err)
	}
	return nil
}
...
func UnarchiveChat(db *sql.DB, slug string) error {
	...
	if err != nil {
		return fmt.Errorf("productdb: unarchive chat %q: %w", slug, err)
	}
	return nil
}
...
func DeleteChat(db *sql.DB, slug, at string) error {
	...
	if err != nil {
		return fmt.Errorf("productdb: delete chat %q: %w", slug, err)
	}
	return nil
}
