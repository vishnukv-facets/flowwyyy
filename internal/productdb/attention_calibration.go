...
func AttentionCalibrationBins(db *sql.DB) ([]AttentionCalibrationBin, error) {
	if db == nil {
		return nil, fmt.Errorf("productdb: attention calibration bins requires db")
	}
	rows, err := db.Query(`
	...
	if err != nil {
		return nil, fmt.Errorf("productdb: attention calibration bins: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&b.Action, &b.ConfidenceBand, &b.Approved, &b.Negative, &b.OperatorHandled); err != nil {
			return nil, fmt.Errorf("productdb: scan attention calibration bin: %w", err)
		}
		out = append(out, b)
	}
	return out, rows.Err()
