...
func RecordAttentionFeedback(db *sql.DB, fb AttentionFeedback) error {
	if db == nil {
		return fmt.Errorf("productdb: record attention feedback requires db")
	}
	...
	if fb.FeedItemID == "" || fb.Source == "" || fb.ThreadKey == "" || fb.SuggestedAction == "" || fb.FinalAction == "" || fb.Outcome == "" {
		return fmt.Errorf("productdb: attention feedback requires feed_item_id, source, thread_key, suggested_action, final_action, outcome")
	}
	if fb.ID == "" {
	...
	if err != nil {
		return fmt.Errorf("productdb: record attention feedback: %w", err)
	}
	return nil
}
...
func RecentAgentReplyDrafts(db *sql.DB, threadKey, since string) ([]string, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("productdb: recent agent reply drafts for %q: %w", threadKey, err)
	}
	defer rows.Close()
	var out []string
	for rows.Next() {
		...
		if err := rows.Scan(&d); err != nil {
			return nil, fmt.Errorf("productdb: scan agent reply draft: %w", err)
		}
		if d.String != "" {
...
func ListAttentionFeedback(db *sql.DB, f AttentionFeedbackFilter) ([]AttentionFeedback, error) {
	...
	rows, err := db.Query(q, args...)
	if err != nil {
		return nil, fmt.Errorf("productdb: list attention feedback: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err != nil {
			return nil, fmt.Errorf("productdb: scan attention feedback: %w", err)
		}
		out = append(out, fb)
	}
	return out, rows.Err()
...
func AttentionFeedbackReport(db *sql.DB, groupBy string) ([]AttentionFeedbackAggregate, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("productdb: attention feedback report: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&row.Group, &row.Total, &row.Approved, &row.Dismissed, &row.Muted); err != nil {
			return nil, fmt.Errorf("productdb: scan attention feedback report: %w", err)
		}
		if row.Total > 0 {
...
func feedbackGroupExpr(groupBy string) (string, error) {
	...
	default:
		return "", fmt.Errorf("productdb: unsupported attention feedback group %q", groupBy)
	}
}
...
func learnSuppressions(db *sql.DB, col string, opts LearnedAttentionPolicyOptions, target map[string]bool) error {
	...
	rows, err := db.Query(`
		...
		GROUP BY value`)
	if err != nil {
		return fmt.Errorf("productdb: learn attention suppressions by %s: %w", col, err)
	}
	defer rows.Close()
	for rows.Next() {
		var value string
		...
		if err := rows.Scan(&value, &total, &negative); err != nil {
			return fmt.Errorf("productdb: scan learned attention suppressions: %w", err)
		}
...
func learnThresholdAdjustments(db *sql.DB, opts LearnedAttentionPolicyOptions, target map[string]float64) error {
	...
	if err != nil {
		return fmt.Errorf("productdb: learn attention threshold adjustments: %w", err)
	}
	defer rows.Close()
	for rows.Next() {
		var action string
		...
		if err := rows.Scan(&action, &total, &approved, &negative); err != nil {
			return fmt.Errorf("productdb: scan learned attention threshold adjustments: %w", err)
		}
