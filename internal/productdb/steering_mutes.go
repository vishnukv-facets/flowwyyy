...
func AddSteeringMute(db *sql.DB, scope, value string) error {
	...
	if scope == "" || value == "" {
		return fmt.Errorf("productdb: steering mute requires scope and value")
	}
	_, err := db.Exec(
	...
	if err != nil {
		return fmt.Errorf("productdb: add steering mute %s=%q: %w", scope, value, err)
	}
	return nil
}
...
func RemoveSteeringMute(db *sql.DB, scope, value string) error {
	...
	if err != nil {
		return fmt.Errorf("productdb: remove steering mute %s=%q: %w", scope, value, err)
	}
	return nil
}
...
func ListSteeringMutes(db *sql.DB) (SteeringMutes, error) {
	...
	if err != nil {
		return out, fmt.Errorf("productdb: list steering mutes: %w", err)
	}
	defer rows.Close()
	for rows.Next() {
		var scope, value string
		if err := rows.Scan(&scope, &value); err != nil {
			return out, fmt.Errorf("productdb: scan steering mute: %w", err)
		}
		switch scope {
