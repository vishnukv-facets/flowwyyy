...
func SetFeedRetriaging(db *sql.DB, id, at string) error {
	if _, err := db.Exec(`UPDATE attention_feed SET retriaging_at = ? WHERE id = ?`, NullIfEmpty(at), id); err != nil {
		return fmt.Errorf("productdb: set feed retriaging %q: %w", id, err)
	}
	return nil
}
...
func UpsertFeedItemSurfaced(db *sql.DB, item FeedItem) (id string, surfaced bool, err error) {
	if item.ID == "" || item.ThreadKey == "" || item.SuggestedAction == "" {
		return "", false, fmt.Errorf("productdb: feed item requires id, thread_key, suggested_action")
	}
	if item.Status == "" {
		item.Status = "new"
	}
	...
	case err != nil:
		return "", false, fmt.Errorf("productdb: lookup feed coalesce: %w", err)
	default:
		...
		if uerr != nil {
			return "", false, fmt.Errorf("productdb: coalesce feed item: %w", uerr)
		}
	...
	if err != nil {
		return "", false, fmt.Errorf("productdb: insert feed item: %w", err)
	}
...
func ListFeedItems(db *sql.DB, status string) ([]FeedItem, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("productdb: list feed items: %w", err)
	}
	defer rows.Close()
...
func scanFeedItemRows(rows *sql.Rows) ([]FeedItem, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			return nil, fmt.Errorf("productdb: scan feed item: %w", err)
		}
...
func ListOpenClubCandidates(db *sql.DB, channel, excludeThreadKey, since string, limit int) ([]FeedItem, error) {
	...
	rows, err := db.Query(q, args...)
	if err != nil {
		return nil, fmt.Errorf("productdb: list club candidates: %w", err)
	}
	defer rows.Close()
	return scanFeedItemRows(rows)
}
...
func ResolveOpenFeedItemsByThread(db *sql.DB, threadKey, actedAt string) (int, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("productdb: resolve open feed items for thread %q: %w", threadKey, err)
	}
...
func LatestFeedItemByThread(db *sql.DB, threadKey string) (FeedItem, bool, error) {
	...
	if err != nil {
		return FeedItem{}, false, fmt.Errorf("productdb: latest feed item for thread %q: %w", threadKey, err)
	}
	defer rows.Close()
...
func SetFeedItemAction(db *sql.DB, id, action, matchedTask string) error {
	...
	if err != nil {
		return fmt.Errorf("productdb: set feed action: %w", err)
	}
	n, _ := res.RowsAffected()
	if n == 0 {
		return fmt.Errorf("productdb: no open feed item with id %q", id)
	}
	return nil
}
...
func SetFeedItemStatus(db *sql.DB, id, status, actedAt string) error {
	...
	if err != nil {
		return fmt.Errorf("productdb: set feed status: %w", err)
	}
	n, _ := res.RowsAffected()
	if n == 0 {
		return fmt.Errorf("productdb: no feed item with id %q", id)
	}
	return nil
}
...
func SetFeedItemActed(db *sql.DB, id, linkedTask, actedAt string) error {
	...
	if err != nil {
		return fmt.Errorf("productdb: set feed acted: %w", err)
	}
	n, _ := res.RowsAffected()
	if n == 0 {
		return fmt.Errorf("productdb: no feed item with id %q", id)
...
func GetFeedItem(db *sql.DB, id string) (FeedItem, error) {
	...
	if err != nil {
		return FeedItem{}, fmt.Errorf("productdb: get feed item %q: %w", id, err)
	}
	it.MatchedTask = matched.String
	it.SuggestedProject = project.String
	it.SuggestedPriority = priority.String
