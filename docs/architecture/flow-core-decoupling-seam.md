...
6. **`product`** — `attention.go` reads cut to productdb; remaining: `ui.go`/`attention.go` still `flowdb.OpenDB` (needs `productdb.Open` + Ensure of the **6 core-gap tables** — step 2's deferred half), and `product` imports `app` for **13 helpers** (FlagSet/ParseFlagSet, FlowRoot/FlowDBPath/UISessionToken/FlowServerURL/PreferredUIFlowBinary, LeadingHelpArg, and the skill machinery EmbeddedCoreSkill/RunSkillCommand/SetEmbeddedSkill/RegisterInitHook) — relocate to a neutral/product home.
